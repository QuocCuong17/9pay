...
public final class _BufferKt {
    ...
    */
    ...
    public static final long commonIndexOf(Buffer buffer, byte b, long j, long j2) {
        ...
        while (j3 < j4) {
            byte[] bArr2 = segment.data;
            int min2 = (int) Math.min(segment.limit, (segment.pos + j4) - j3);
            i = (int) ((segment.pos + j) - j3);
            while (i < min2) {
                if (bArr2[i] != b) {
                    i++;
    ...
    public static final long commonIndexOf(Buffer buffer, ByteString bytes, long j) {
        ...
        if (buffer.size() - j < j) {
            ...
            loop1: while (j2 < size3) {
                ...
                if (i < min) {
                    while (true) {
                        int i2 = i + 1;
                        if (bArr[i] == b && rangeEquals(segment, i2, internalArray$okio, 1, size2)) {
                            break loop1;
                        }
                        if (i2 >= min) {
                            break;
        ...
        loop4: while (j2 < size5) {
            ...
            int min2 = (int) Math.min(segment3.limit, (segment3.pos + size5) - j2);
            segment = segment3;
            ...
            if (i < min2) {
                while (true) {
                    int i3 = i + 1;
                    z = true;
                    if (bArr2[i] == b2 && rangeEquals(segment, i3, internalArray$okio2, 1, size4)) {
                        break loop4;
                    }
                    if (i3 >= min2) {
                        break;
