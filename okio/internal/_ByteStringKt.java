...
public final class _ByteStringKt {
    ...
    public static final ByteString commonSubstring(ByteString byteString, int i, int i2) {
        ...
        if (resolveDefaultParameter <= byteString.getData().length) {
            if (resolveDefaultParameter - i >= 0) {
                return (i == 0 && resolveDefaultParameter == byteString.getData().length) ? byteString : new ByteString(ArraysKt.copyOfRange(byteString.getData(), i, resolveDefaultParameter));
            }
    ...
    public static final int commonLastIndexOf(ByteString byteString, byte[] other, int i) {
        Intrinsics.checkNotNullParameter(byteString, "<this>");
        Intrinsics.checkNotNullParameter(other, "other");
        int min = Math.min(_UtilKt.resolveDefaultParameter(byteString, i), byteString.getData().length - other.length);
        if (min < 0) {
            return -1;
        }
        while (true) {
    ...
    public static final int codePointIndexToCharIndex(byte[] bArr, int i) {
        ...
        loop0: while (i3 < length) {
            ...
            if ((b >> 5) == -2) {
                ...
                int i13 = (((b11 ^ 3678080) ^ (b10 << 6)) ^ (b9 << 12)) ^ (b8 << Ascii.DC2);
                if (i13 > 1114111) {
                    if (i5 == i) {
                        return i4;
                    }
