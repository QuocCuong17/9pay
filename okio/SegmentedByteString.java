...
public final class SegmentedByteString extends ByteString {
    ...
    public ByteString substring(int beginIndex, int endIndex) {
        ...
        int resolveDefaultParameter = _UtilKt.resolveDefaultParameter(segmentedByteString, endIndex);
        if (!(beginIndex >= 0)) {
    ...
    public boolean rangeEquals(int offset, byte[] other, int otherOffset, int byteCount) {
        ...
        while (offset < i) {
            ...
            int i4 = getDirectory()[getSegments().length + segment];
            int min = Math.min(i, i3 + i2) - offset;
            if (!_UtilKt.arrayRangeEquals(getSegments()[segment], i4 + (offset - i2), other, otherOffset, min)) {
                return false;
            }
            otherOffset += min;
            offset += min;
