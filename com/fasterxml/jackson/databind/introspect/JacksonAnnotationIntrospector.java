...
public class JacksonAnnotationIntrospector extends AnnotationIntrospector implements Serializable {
    ...
    public PropertyName findRootName(AnnotatedClass annotatedClass) {
        ...
        return PropertyName.construct(jsonRootName.value(), (namespace == null || !namespace.isEmpty()) ? namespace : null);
    }
    ...
    public JavaType refineSerializationType(MapperConfig<?> mapperConfig, Annotated annotated, JavaType javaType) throws JsonMappingException {
        ...
        if (_classIfExplicit != null) {
            if (javaType.hasRawClass(_classIfExplicit)) {
                ...
                try {
                    ...
                    throw _databindException(e, String.format("Failed to widen type %s with annotation (value %s), from '%s': %s", javaType, _classIfExplicit.getName(), annotated.getName(), e.getMessage()));
                }
            }
        }
            ...
            if (_classIfExplicit2 != null) {
                if (keyType.hasRawClass(_classIfExplicit2)) {
                    ...
                    try {
                        ...
                        throw _databindException(e2, String.format("Failed to widen key type of %s with concrete-type annotation (value %s), from '%s': %s", javaType, _classIfExplicit2.getName(), annotated.getName(), e2.getMessage()));
                    }
                }
        ...
        if (contentType.hasRawClass(_classIfExplicit3)) {
            ...
            try {
                ...
                throw _databindException(e3, String.format("Internal error: failed to refine value type of %s with concrete-type annotation (value %s), from '%s': %s", javaType, _classIfExplicit3.getName(), annotated.getName(), e3.getMessage()));
            }
        }
    ...
    public JavaType refineDeserializationType(MapperConfig<?> mapperConfig, Annotated annotated, JavaType javaType) throws JsonMappingException {
        ...
        if (_classIfExplicit != null && !javaType.hasRawClass(_classIfExplicit) && !_primitiveAndWrapper(javaType, _classIfExplicit)) {
            try {
                ...
                throw _databindException(e, String.format("Failed to narrow type %s with annotation (value %s), from '%s': %s", javaType, _classIfExplicit.getName(), annotated.getName(), e.getMessage()));
            }
        }
        if (javaType.isMapLikeType()) {
            JavaType keyType = javaType.getKeyType();
            ...
            if (_classIfExplicit2 != null && !_primitiveAndWrapper(keyType, _classIfExplicit2)) {
                try {
                    ...
                    throw _databindException(e2, String.format("Failed to narrow key type of %s with concrete-type annotation (value %s), from '%s': %s", javaType, _classIfExplicit2.getName(), annotated.getName(), e2.getMessage()));
                }
            }
        }
        JavaType contentType = javaType.getContentType();
        ...
        Class<?> _classIfExplicit3 = jsonDeserialize != null ? _classIfExplicit(jsonDeserialize.contentAs()) : null;
        ...
        try {
            ...
            throw _databindException(e3, String.format("Failed to narrow value type of %s with concrete-type annotation (value %s), from '%s': %s", javaType, _classIfExplicit3.getName(), annotated.getName(), e3.getMessage()));
        }
    }
