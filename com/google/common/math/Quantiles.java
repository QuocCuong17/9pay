...
public final class Quantiles {
        ...
        public Map<Integer, Double> computeInPlace(double... dataset) {
            ...
            while (true) {
                ...
                if (i < iArr6.length) {
                    ...
                    int i6 = iArr4[i];
                    if (i6 == 0) {
                        linkedHashMap2.put(Integer.valueOf(iArr6[i]), Double.valueOf(dataset[i5]));
                    } else {
                        linkedHashMap2.put(Integer.valueOf(iArr6[i]), Double.valueOf(Quantiles.interpolate(dataset[i5], dataset[i5 + 1], i6, this.scale)));
                    }
                    i++;
                } else {
    ...
    private static void movePivotToStartOfSlice(double[] array, int from, int to) {
        int i = (from + to) >>> 1;
        boolean z = array[to] < array[i];
        boolean z2 = array[i] < array[from];
        boolean z3 = array[to] < array[from];
        if (z == z2) {
            swap(array, i, from);
        } else if (z != z3) {
            swap(array, from, to);
