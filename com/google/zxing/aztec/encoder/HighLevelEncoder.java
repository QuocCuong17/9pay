    ...
    */
    public BitArray encode() {
        ...
        if (charset != null) {
            ...
            state = state.appendFLGn(characterSetECI.getValue());
        }
        Collection<State> singletonList = Collections.singletonList(state);
        int i2 = 0;
        while (true) {
            byte[] bArr = this.text;
            if (i2 < bArr.length) {
                ...
                return ((State) Collections.min(singletonList, new Comparator<State>() { // from class: com.google.zxing.aztec.encoder.HighLevelEncoder.1
                    ...
                    public int compare(State state2, State state3) {
                        return state2.getBitCount() - state3.getBitCount();
                    }
