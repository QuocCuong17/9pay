...
public final class Encoder {
    ...
    */
    public static QRCode encode(String str, ErrorCorrectionLevel errorCorrectionLevel, Map<EncodeHintType, ?> map) throws WriterException {
        ...
        Charset forName = z3 ? Charset.forName(map.get(EncodeHintType.CHARACTER_SET).toString()) : obj;
        if (z2) {
            mode = Mode.BYTE;
            if (forName.equals(obj)) {
                forName = null;
            }
            MinimalEncoder.ResultList encode = MinimalEncoder.encode(str, null, forName, z, errorCorrectionLevel);
            bitArray = new BitArray();
            encode.getBits(bitArray);
            version = encode.getVersion();
        } else {
            Mode chooseMode = chooseMode(str, forName);
            BitArray bitArray2 = new BitArray();
            if (chooseMode == Mode.BYTE && z3 && (characterSetECI = CharacterSetECI.getCharacterSetECI(forName)) != null) {
            ...
            appendBytes(str, chooseMode, bitArray3, forName);
            ...
            appendLengthInfo(chooseMode == Mode.BYTE ? bitArray3.getSizeInBytes() : str.length(), recommendVersion, chooseMode, bitArray4);
