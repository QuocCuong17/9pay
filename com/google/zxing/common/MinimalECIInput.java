    ...
    static int[] encodeMinimally(String str, ECIEncoderSet eCIEncoderSet, int i) {
        ...
        for (int i2 = 1; i2 <= length; i2++) {
            for (int i3 = 0; i3 < eCIEncoderSet.length(); i3++) {
                if (inputEdgeArr[i2][i3] != null && i2 < length) {
                    addEdges(str, eCIEncoderSet, inputEdgeArr, i2, inputEdgeArr[i2][i3], i);
                }
            }
        ...
        for (int i7 = 0; i7 < eCIEncoderSet.length(); i7++) {
            if (inputEdgeArr[length][i7] != null) {
                InputEdge inputEdge = inputEdgeArr[length][i7];
                if (inputEdge.cachedTotalSize < i5) {
                    i5 = inputEdge.cachedTotalSize;
                    i6 = i7;
                }
            }
        }
        ...
        for (InputEdge inputEdge2 = inputEdgeArr[length][i6]; inputEdge2 != null; inputEdge2 = inputEdge2.previous) {
            if (inputEdge2.isFNC1()) {
                arrayList.add(0, 1000);
            } else {
                byte[] encode = eCIEncoderSet.encode(inputEdge2.c, inputEdge2.encoderIndex);
