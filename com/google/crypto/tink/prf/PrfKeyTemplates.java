...
public final class PrfKeyTemplates {
    ...
    private static KeyTemplate createHmacTemplate(int keySize, HashType hashType) {
        return (KeyTemplate) KeyTemplate.newBuilder().setTypeUrl(new HmacPrfKeyManager().getKeyType()).setValue(((HmacPrfKeyFormat) HmacPrfKeyFormat.newBuilder().setParams((HmacPrfParams) HmacPrfParams.newBuilder().setHash(hashType).build()).setKeySize(keySize).build()).toByteString()).setOutputPrefixType(OutputPrefixType.RAW).build();
    }
