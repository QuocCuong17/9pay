...
public final class zzi {
    ...
    public static AssetFileDescriptor zza(Context context, Uri uri, String str) throws FileNotFoundException {
        ...
        if ("android.resource".equals(scheme)) {
            return contentResolver.openAssetFileDescriptor(zzc2, PDPageLabelRange.STYLE_ROMAN_LOWER);
        }
        if (FirebaseAnalytics.Param.CONTENT.equals(scheme)) {
            if (!zzi(context, zzc2, 1, zzhVar)) {
                throw new FileNotFoundException("Can't open content uri.");
            }
            AssetFileDescriptor openAssetFileDescriptor = contentResolver.openAssetFileDescriptor(zzc2, PDPageLabelRange.STYLE_ROMAN_LOWER);
            zzd(openAssetFileDescriptor);
            return openAssetFileDescriptor;
        }
        if ("file".equals(scheme)) {
            AssetFileDescriptor openAssetFileDescriptor2 = contentResolver.openAssetFileDescriptor(zzc2, PDPageLabelRange.STYLE_ROMAN_LOWER);
    ...
    public static InputStream zzb(Context context, Uri uri) throws FileNotFoundException {
        ...
        if ("file".equals(scheme)) {
            try {
                ParcelFileDescriptor openFileDescriptor = contentResolver.openFileDescriptor(Uri.fromFile(new File(zzc2.getPath()).getCanonicalFile()), PDPageLabelRange.STYLE_ROMAN_LOWER);
                try {
