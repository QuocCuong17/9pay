...
public final class CctTransportBackend implements TransportBackend {
    ...
    private static TelephonyManager getTelephonyManager(Context context) {
        return (TelephonyManager) context.getSystemService(WorkflowModule.Properties.Section.Component.Keyboard.PHONE);
    }
    ...
    public EventInternal decorate(EventInternal eventInternal) {
        ...
        return eventInternal.toBuilder().addMetadata("sdk-version", Build.VERSION.SDK_INT).addMetadata("model", Build.MODEL).addMetadata(KEY_HARDWARE, Build.HARDWARE).addMetadata("device", Build.DEVICE).addMetadata(KEY_PRODUCT, Build.PRODUCT).addMetadata(KEY_OS_BUILD, Build.ID).addMetadata("manufacturer", Build.MANUFACTURER).addMetadata("fingerprint", Build.FINGERPRINT).addMetadata(KEY_TIMEZONE_OFFSET, getTzOffset()).addMetadata(KEY_NETWORK_TYPE, getNetTypeValue(activeNetworkInfo)).addMetadata(KEY_MOBILE_SUBTYPE, getNetSubtypeValue(activeNetworkInfo)).addMetadata("country", Locale.getDefault().getCountry()).addMetadata("locale", Locale.getDefault().getLanguage()).addMetadata(KEY_MCC_MNC, getTelephonyManager(this.applicationContext).getSimOperator()).addMetadata(KEY_APPLICATION_BUILD, Integer.toString(getPackageVersionCode(this.applicationContext))).build();
    }
    ...
    private BatchedLogRequest getRequestBody(BackendRequest backendRequest) {
        ...
        for (Map.Entry entry : hashMap.entrySet()) {
            ...
            for (EventInternal eventInternal3 : (List) entry.getValue()) {
                ...
                } else if (encoding.equals(Encoding.of(WorkflowRequestType.JSON))) {
