...
public class RequestCreator {
        ...
        if (this.data.hasImage()) {
            ...
            if (MemoryPolicy.shouldReadFromMemoryCache(this.memoryPolicy) && this.picasso.quickMemoryCacheCheck(createKey) != null) {
                if (this.picasso.loggingEnabled) {
                    Utils.log("Main", WorkflowModule.Properties.Section.Component.State.STATE_COMPLETED, createRequest.plainId(), "from " + Picasso.LoadedFrom.MEMORY);
                }
                if (callback != null) {
                    callback.onSuccess();
                    return;
    ...
    private Request createRequest(long j) {
        ...
        if (transformRequest != build) {
            transformRequest.f112id = andIncrement;
