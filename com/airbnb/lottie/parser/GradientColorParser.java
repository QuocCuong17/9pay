    ...
    private int getColorInBetweenColorStops(float f, float f2, float[] fArr, int[] iArr) {
        ...
        for (int i = 1; i < fArr.length; i++) {
            if (fArr[i] >= f || i == fArr.length - 1) {
                ...
                int i3 = iArr[i];
                int i4 = iArr[i2];
                return Color.argb((int) (f2 * 255.0f), MiscUtils.lerp(Color.red(i4), Color.red(i3), f3), MiscUtils.lerp(Color.green(i4), Color.green(i3), f3), MiscUtils.lerp(Color.blue(i4), Color.blue(i3), f3));
            }
        }
        ...
        for (int i2 = 1; i2 < fArr.length; i2++) {
            ...
            if (f2 >= f || i2 == fArr.length - 1) {
                if (f2 <= f) {
                    ...
                    int i3 = i2 - 1;
                    lerp = MiscUtils.lerp(fArr2[i3], fArr2[i2], (f - fArr[i3]) / (fArr[i2] - fArr[i3]));
                }
