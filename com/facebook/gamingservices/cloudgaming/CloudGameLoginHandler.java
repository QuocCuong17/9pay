...
public class CloudGameLoginHandler {
    ...
    public static synchronized AccessToken init(Context context, int i) throws FacebookException {
        ...
        synchronized (CloudGameLoginHandler.class) {
            ...
            setPackageName(executeAndWait.getGraphObject(), context);
            try {
                currentAccessToken = setCurrentAccessToken(executeAndWait.getGraphObject());
                GamingPayload.loadPayloadFromCloudGame(executeAndWait.getGraphObject().optString("payload"));
    ...
    private static boolean isCloudEnvReady(Context context, int i) {
        ...
        return (executeAndWait == null || executeAndWait.getGraphObject() == null || executeAndWait.getError() != null) ? false : true;
    }
