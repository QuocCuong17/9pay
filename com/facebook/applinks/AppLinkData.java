...
public class AppLinkData {
    ...
    public static void fetchDeferredAppLinkFromServer(Context context, String str, CompletionHandler completionHandler) {
        ...
        try {
            ...
            try {
                JSONObject graphObject = GraphRequest.newPostRequest(null, String.format(DEFERRED_APP_LINK_PATH, objArr), jSONObject, null).executeAndWait().getGraphObject();
                if (graphObject != null) {
                    String optString = graphObject.optString(DEFERRED_APP_LINK_ARGS_FIELD);
                    long optLong = graphObject.optLong(DEFERRED_APP_LINK_CLICK_TIME_FIELD, -1L);
                    String optString2 = graphObject.optString(DEFERRED_APP_LINK_CLASS_FIELD);
                    String optString3 = graphObject.optString(DEFERRED_APP_LINK_URL_FIELD);
                    if (!TextUtils.isEmpty(optString) && (appLinkData = createFromJson(optString)) != null) {
                        if (optLong != -1) {
                            try {
                                JSONObject jSONObject2 = appLinkData.arguments;
                                if (jSONObject2 != null) {
