    ...
    public static final <S extends Segment<S>> Object findSegmentInternal(S s, long j, Function2<? super Long, ? super S, ? extends S> function2) {
        while (true) {
            if (s.id >= j && !s.isRemoved()) {
                return SegmentOrClosed.m2764constructorimpl(s);
            }
            Object nextOrClosed = s.getNextOrClosed();
            if (nextOrClosed == CLOSED) {
                return SegmentOrClosed.m2764constructorimpl(CLOSED);
            }
        ...
        do {
            ...
            if (!SegmentOrClosed.m2769isClosedimpl(findSegmentInternal)) {
                ...
                while (true) {
                    Segment segment2 = (Segment) atomicReferenceFieldUpdater.get(obj);
                    z = true;
                    if (segment2.id >= m2767getSegmentimpl.id) {
                        break;
                    }
                    ...
                    if (atomicReferenceFieldUpdater.compareAndSet(obj, segment2, m2767getSegmentimpl)) {
                        if (segment2.decPointers$kotlinx_coroutines_core()) {
                            segment2.remove();
                        }
        ...
        do {
            ...
            if (!SegmentOrClosed.m2769isClosedimpl(findSegmentInternal)) {
                ...
                while (true) {
                    ...
                    if (atomicReferenceArray.compareAndSet(i, segment2, m2767getSegmentimpl)) {
                        if (segment2.decPointers$kotlinx_coroutines_core()) {
                            segment2.remove();
                        }
                    } else if (m2767getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
