...
public final class TaskLoggerKt {
    ...
    public static final <T> T logElapsed(Task task, TaskQueue queue, Function0<? extends T> block) {
        ...
        try {
            ...
            if (isLoggable) {
                log(task, queue, Intrinsics.stringPlus("finished run in ", formatDuration(queue.getTaskRunner().getBackend().nanoTime() - j)));
            }
            ...
            InlineMarker.finallyStart(1);
            if (isLoggable) {
                log(task, queue, Intrinsics.stringPlus("failed a run in ", formatDuration(queue.getTaskRunner().getBackend().nanoTime() - j)));
            }
            InlineMarker.finallyEnd(1);
            throw th;
        }
