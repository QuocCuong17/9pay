...
public class GMSSSigner implements MessageSigner {
    ...
    private void initSign() {
        ...
        while (true) {
            ...
            this.currentAuthPaths[i3] = (byte[][]) Array.newInstance((Class<?>) byte.class, currentAuthPaths[i3].length, this.mdLength);
            for (int i4 = 0; i4 < currentAuthPaths[i3].length; i4++) {
                System.arraycopy(currentAuthPaths[i3][i4], 0, this.currentAuthPaths[i3][i4], 0, this.mdLength);
            }
            i3++;
        }
    ...
    public boolean verifySignature(byte[] bArr, byte[] bArr2) {
        ...
        for (int i2 = this.numLayer - 1; i2 >= 0; i2--) {
            ...
            byte[][] bArr4 = (byte[][]) Array.newInstance((Class<?>) byte.class, this.gmssPS.getHeightOfTrees()[i2], this.mdLength);
