...
public class IOUtils {
    ...
    public static long skip(InputStream inputStream, long j) throws IOException {
        ...
        long j2 = j;
        while (j2 > 0) {
            long read = inputStream.read(SKIP_BYTE_BUFFER, 0, (int) Math.min(j2, r4.length));
            if (read < 0) {
                break;
            }
            j2 -= read;
    ...
    public static long skip(ReadableByteChannel readableByteChannel, long j) throws IOException {
        ...
        ByteBuffer allocate = ByteBuffer.allocate((int) Math.min(j, SKIP_BYTE_BUFFER.length));
        long j2 = j;
        while (j2 > 0) {
            allocate.position(0);
            allocate.limit((int) Math.min(j2, SKIP_BYTE_BUFFER.length));
    ...
    public static long skip(Reader reader, long j) throws IOException {
        ...
        long j2 = j;
        while (j2 > 0) {
            long read = reader.read(SKIP_CHAR_BUFFER, 0, (int) Math.min(j2, SKIP_BYTE_BUFFER.length));
            if (read < 0) {
                break;
            }
            j2 -= read;
