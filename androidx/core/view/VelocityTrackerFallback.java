...
class VelocityTrackerFallback {
    ...
    private float getCurrentVelocity() {
        ...
        int i3 = ((i2 + 20) - (i - 1)) % 20;
        long j = this.mEventTimes[i2];
        while (true) {
            jArr = this.mEventTimes;
            if (j - jArr[i3] <= RANGE_MS) {
                break;
            }
        ...
        if (i4 == 2) {
            ...
            return this.mMovements[i5] / ((float) (jArr[i5] - jArr[i3]));
        }
        int i6 = 0;
        for (int i7 = 0; i7 < this.mDataPointsBufferSize - 1; i7++) {
            ...
            if (jArr2[i9] != j2) {
                ...
                float f2 = this.mMovements[i9] / ((float) (this.mEventTimes[i9] - j2));
