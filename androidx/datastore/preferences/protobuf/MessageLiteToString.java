    ...
    private static void reflectivePrintWithIndent(MessageLite messageLite, StringBuilder sb, int i) {
        ...
        for (java.lang.reflect.Method method : messageLite.getClass().getDeclaredMethods()) {
            ...
            if (method.getParameterTypes().length == 0) {
                ...
                if (method.getName().startsWith(WorkflowRequestType.Method.GET)) {
                    treeSet.add(method.getName());
                }
            }
        }
        for (String str : treeSet) {
            String replaceFirst = str.replaceFirst(WorkflowRequestType.Method.GET, "");
            boolean z = true;
            if (replaceFirst.endsWith("List") && !replaceFirst.endsWith(BUILDER_LIST_SUFFIX) && !replaceFirst.equals("List")) {
            ...
            if (((java.lang.reflect.Method) hashMap2.get("set" + replaceFirst)) != null) {
                if (replaceFirst.endsWith(BYTES_SUFFIX)) {
                    if (hashMap.containsKey(WorkflowRequestType.Method.GET + replaceFirst.substring(0, replaceFirst.length() - 5))) {
                    }
                }
                String str4 = replaceFirst.substring(0, 1).toLowerCase() + replaceFirst.substring(1);
                java.lang.reflect.Method method4 = (java.lang.reflect.Method) hashMap.get(WorkflowRequestType.Method.GET + replaceFirst);
