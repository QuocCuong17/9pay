...
final class WavHeaderReader {
    ...
    public static boolean checkFileType(ExtractorInput extractorInput) throws IOException {
        ...
        if (peek.f46id != 1380533830 && peek.f46id != 1380333108) {
            return false;
        }
    ...
    private static ChunkHeader skipToChunk(int i, ExtractorInput extractorInput, ParsableByteArray parsableByteArray) throws IOException {
        ChunkHeader peek = ChunkHeader.peek(extractorInput, parsableByteArray);
        while (peek.f46id != i) {
            Log.w(TAG, "Ignoring unknown WAV chunk: " + peek.f46id);
            long j = peek.size + 8;
            if (peek.size % 2 != 0) {
                j++;
            }
            if (j > 2147483647L) {
                throw ParserException.createForUnsupportedContainerFeature("Chunk is too large (~2GB+) to skip; id: " + peek.f46id);
            }
