...
public final class DtsUtil {
    ...
    private static final int[] TWICE_BITRATE_KBPS_BY_RATE = {64, 112, 128, 192, 224, 256, RendererCapabilities.DECODER_SUPPORT_MASK, 448, 512, 640, ValidationPath.MAX_PATH_LENGTH_BYTES, 896, 1024, 1152, PlatformPlugin.DEFAULT_SYSTEM_UI, 1536, 1920, 2048, 2304, 2560, 2688, 2816, 2823, 2944, 3072, 3840, 4096, 6144, 7680};
    ...
    */
    ...
    public static DtsHeader parseDtsUhdHeader(byte[] bArr, AtomicInteger atomicInteger) throws ParserException {
        ...
        for (int i6 = 0; i6 < i4; i6++) {
            i5 += parseUnsignedVarInt(normalizedFrame, UHD_METADATA_CHUNK_SIZE_LENGTH_TABLE, true);
        }
        for (int i7 = 0; i7 < 1; i7++) {
            if (i4 != 0) {
                atomicInteger.set(parseUnsignedVarInt(normalizedFrame, UHD_AUDIO_CHUNK_ID_LENGTH_TABLE, true));
            }
            i5 += atomicInteger.get() != 0 ? parseUnsignedVarInt(normalizedFrame, UHD_AUDIO_CHUNK_SIZE_LENGTH_TABLE, true) : 0;
        }
        return new DtsHeader(MimeTypes.AUDIO_DTS_X, 2, i, parseUnsignedVarInt + i5, j2, 0);
    }
    ...
    private static ParsableBitArray getNormalizedFrame(byte[] bArr) {
        if (bArr[0] == Byte.MAX_VALUE || bArr[0] == 100 || bArr[0] == 64 || bArr[0] == 113) {
    ...
    private static boolean isLittleEndianFrameHeader(byte[] bArr) {
        return bArr[0] == -2 || bArr[0] == -1 || bArr[0] == 37 || bArr[0] == -14 || bArr[0] == -24;
    }
