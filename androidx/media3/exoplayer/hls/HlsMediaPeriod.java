...
public final class HlsMediaPeriod implements MediaPeriod, HlsPlaylistTracker.PlaylistEventListener {
    ...
    private void buildAndPrepareSampleStreamWrappers(long j) {
        ...
        while (i < list2.size()) {
            ...
            int i2 = i;
            HlsSampleStreamWrapper buildSampleStreamWrapper = buildSampleStreamWrapper(str, 3, new Uri[]{rendition.url}, new Format[]{format}, null, Collections.emptyList(), map, j);
    ...
    private void buildAndPrepareMainSampleStreamWrapper(HlsMultivariantPlaylist hlsMultivariantPlaylist, long j, List<HlsSampleStreamWrapper> list, List<int[]> list2, Map<String, DrmInitData> map) {
        ...
        HlsSampleStreamWrapper buildSampleStreamWrapper = buildSampleStreamWrapper(SentryThread.JsonKeys.MAIN, (z || codecCountOfType2 <= 0) ? 0 : 1, uriArr, formatArr, hlsMultivariantPlaylist.muxedAudioFormat, hlsMultivariantPlaylist.muxedCaptionFormats, map, j);
        ...
        if (this.allowChunklessPreparation && z3) {
            ...
            if (codecCountOfType > 0) {
                ...
                if (codecCountOfType2 > 0 && (hlsMultivariantPlaylist.muxedAudioFormat != null || hlsMultivariantPlaylist.audios.isEmpty())) {
                    arrayList.add(new TrackGroup(SentryThread.JsonKeys.MAIN + ":audio", deriveAudioFormat(formatArr[0], hlsMultivariantPlaylist.muxedAudioFormat, false)));
                }
                List<Format> list3 = hlsMultivariantPlaylist.muxedCaptionFormats;
                if (list3 != null) {
                    for (int i8 = 0; i8 < list3.size(); i8++) {
                        arrayList.add(new TrackGroup(SentryThread.JsonKeys.MAIN + ":cc:" + i8, this.extractorFactory.getOutputTextFormat(list3.get(i8))));
                    }
                }
            } else {
    ...
    private static Format deriveAudioFormat(Format format, Format format2, boolean z) {
        ...
        if (format2 != null) {
            ...
            if (z) {
                ...
                str2 = codecsOfType;
                list = of;
                return new Format.Builder().setId(format.f17id).setLabel(str3).setLabels(list).setContainerMimeType(format.containerMimeType).setSampleMimeType(MimeTypes.getMediaMimeType(str2)).setCodecs(str2).setMetadata(metadata).setAverageBitrate(!z ? format.averageBitrate : -1).setPeakBitrate(z ? format.peakBitrate : -1).setChannelCount(i3).setSelectionFlags(i).setRoleFlags(i2).setLanguage(str).build();
            }
        }
        i = i4;
        if (!z) {
