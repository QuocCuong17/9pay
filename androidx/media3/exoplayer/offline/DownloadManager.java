...
public final class DownloadManager {
    ...
    public static final class InternalHandler extends Handler {
        ...
        private void addDownload(DownloadRequest downloadRequest, int i) {
            Download download = getDownload(downloadRequest.f30id, true);
        ...
        private void syncTasks() {
            ...
            for (int i2 = 0; i2 < this.downloads.size(); i2++) {
                ...
                Task task = this.activeTasks.get(download.request.f30id);
                int i3 = download.state;
                if (i3 == 0) {
        ...
        private Task syncQueuedDownload(Task task, Download download) {
            if (task == null) {
                ...
                this.activeTasks.put(putDownloadWithState.request.f30id, task2);
        ...
        private void syncRemovingDownload(Task task, Download download) {
            if (task == null) {
                ...
                this.activeTasks.put(download.request.f30id, task2);
        ...
        private void onContentLengthChanged(Task task, long j) {
            Download download = (Download) Assertions.checkNotNull(getDownload(task.request.f30id, false));
        ...
        private void onDownloadTaskStopped(Download download, Exception exc) {
            ...
            this.downloads.remove(getDownloadIndex(download2.request.f30id));
            try {
        ...
        private void onRemoveTaskStopped(Download download) {
            if (download.state == 7) {
                ...
                this.downloads.remove(getDownloadIndex(download.request.f30id));
                try {
                    this.downloadIndex.removeDownload(download.request.f30id);
                } catch (IOException unused) {
        ...
        private Download putDownload(Download download) {
            ...
            int downloadIndex = getDownloadIndex(download.request.f30id);
            if (downloadIndex == -1) {
        ...
        private int getDownloadIndex(String str) {
            for (int i = 0; i < this.downloads.size(); i++) {
                if (this.downloads.get(i).request.f30id.equals(str)) {
                    return i;
                }
            }
            return -1;
