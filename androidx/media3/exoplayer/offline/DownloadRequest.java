...
public final class DownloadRequest implements Parcelable {
    ...
    DownloadRequest(Parcel parcel) {
        this.f30id = (String) Util.castNonNull(parcel.readString());
    ...
    public DownloadRequest copyWithKeySetId(byte[] bArr) {
        return new DownloadRequest(this.f30id, this.uri, this.mimeType, this.streamKeys, bArr, this.customCacheKey, this.data);
    }

    public DownloadRequest copyWithMergedRequest(DownloadRequest downloadRequest) {
        List emptyList;
        Assertions.checkArgument(this.f30id.equals(downloadRequest.f30id));
        ...
        return new DownloadRequest(this.f30id, downloadRequest.uri, downloadRequest.mimeType, emptyList, downloadRequest.keySetId, downloadRequest.customCacheKey, downloadRequest.data);
    }

    public MediaItem toMediaItem() {
        return new MediaItem.Builder().setMediaId(this.f30id).setUri(this.uri).setCustomCacheKey(this.customCacheKey).setMimeType(this.mimeType).setStreamKeys(this.streamKeys).build();
    }

    public String toString() {
        return this.mimeType + ":" + this.f30id;
    }
