...
public final class SpeedChangingAudioProcessor extends BaseAudioProcessor {
    ...
    private void updateLastProcessedInputTime() {
        synchronized (this.lock) {
            if (isUsingSonic()) {
                this.lastProcessedInputTimeUs = this.inputSegmentStartTimesUs.get(r3.size() - 1) + Util.scaleLargeTimestamp(this.sonicAudioProcessor.getProcessedInputBytes(), 1000000L, this.inputAudioFormat.sampleRate * this.inputAudioFormat.bytesPerFrame);
            } else {
                this.lastProcessedInputTimeUs = Util.scaleLargeTimestamp(this.bytesRead, 1000000L, this.inputAudioFormat.sampleRate * this.inputAudioFormat.bytesPerFrame);
            }
        }
    }
    ...
    @RequiresNonNull({SentryStackFrame.JsonKeys.LOCK})
