...
public class ProfileInstallReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (ACTION_INSTALL_PROFILE.equals(action)) {
            ProfileInstaller.writeProfile(context, DashDownloader$$ExternalSyntheticLambda0.INSTANCE, new ResultDiagnostics(), true);
            return;
        }
        if (ACTION_SKIP_FILE.equals(action)) {
            ...
            if (extras2 != null) {
                ...
                if (EXTRA_SKIP_FILE_OPERATION_WRITE.equals(string)) {
                    ProfileInstaller.writeSkipFile(context, DashDownloader$$ExternalSyntheticLambda0.INSTANCE, new ResultDiagnostics());
                    return;
                } else {
                    if (EXTRA_SKIP_FILE_OPERATION_DELETE.equals(string)) {
                        ProfileInstaller.deleteSkipFile(context, DashDownloader$$ExternalSyntheticLambda0.INSTANCE, new ResultDiagnostics());
                        return;
                    }
                    return;
                }
