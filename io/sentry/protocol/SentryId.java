...
public final class SentryId implements JsonSerializable {
    ...
    public String toString() {
        return StringUtils.normalizeUUID(this.uuid.toString()).replace(Parameters.DEFAULT_OPTION_PREFIXES, "");
    }
    ...
    private UUID fromStringSentryId(String str) {
        if (str.length() == 32) {
            str = new StringBuilder(str).insert(8, Parameters.DEFAULT_OPTION_PREFIXES).insert(13, Parameters.DEFAULT_OPTION_PREFIXES).insert(18, Parameters.DEFAULT_OPTION_PREFIXES).insert(23, Parameters.DEFAULT_OPTION_PREFIXES).toString();
        }
        if (str.length() != 36) {
