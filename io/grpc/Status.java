...
public final class Status {
    ...
    private static Status fromCodeValueSlow(byte[] bArr) {
        ...
        char c = 1;
        if (length != 1) {
            i = (length == 2 && bArr[0] >= 48 && bArr[0] <= 57) ? 0 + ((bArr[0] - ISO7816.INS_DECREASE) * 10) : 0;
            return UNKNOWN.withDescription("Unknown code " + new String(bArr, Charsets.US_ASCII));
        }
        c = 0;
        if (bArr[c] >= 48 && bArr[c] <= 57) {
            int i2 = i + (bArr[c] - ISO7816.INS_DECREASE);
            List<Status> list = STATUS_LIST;
            if (i2 < list.size()) {
                return list.get(i2);
            }
    ...
    public Status augmentDescription(String str) {
        ...
        return new Status(this.code, this.description + IOUtils.LINE_SEPARATOR_UNIX + str, this.cause);
    }
    ...
    private static final class StatusMessageMarshaller implements Metadata.TrustedAsciiMarshaller<String> {
        private static final byte[] HEX = {ISO7816.INS_DECREASE, 49, ISO7816.INS_INCREASE, 51, ISO7816.INS_DECREASE_STAMPED, 53, 54, 55, 56, 57, 65, 66, 67, ISO7816.INS_REHABILITATE_CHV, 69, 70};
